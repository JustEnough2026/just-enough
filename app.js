...
products.forEach(p=>{
 ...
 n.querySelector("h3").textContent=p.name;n.querySelector(".desc").textContent=p.desc;n.querySelector(".price").textContent="$"+p.price;
 state[p.id]??={qty:0,option:p.options?.[0]||""};n.querySelector(".qty").textContent=state[p.id].qty;
 const opts=n.querySelector(".options");
 (p.options||[]).forEach(o=>{const b=document.createElement("button");b.className="option"+(state[p.id].option===o?" active":"");b.textContent=o;b.onclick=()=>{state[p.id].option=o;opts.querySelectorAll(".option").forEach(x=>x.classList.remove("active"));b.classList.add("active");save()};opts.appendChild(b)});
 n.querySelector(".plus").onclick=()=>{state[p.id].qty++;card.querySelector(".qty").textContent=state[p.id].qty;save()};
 n.querySelector(".minus").onclick=()=>{state[p.id].qty=Math.max(0,state[p.id].qty-1);card.querySelector(".qty").textContent=state[p.id].qty;save()};
 list.appendChild(n);
});
