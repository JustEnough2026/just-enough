...
{id:"noodle-s-spicy",name:"小涼粉（酸辣）",desc:"爽口彈滑",price:60,img:"noodles_small.jpg"},
{id:"noodle-s-sesame",name:"小涼粉（胡麻）",desc:"爽口彈滑",price:60,img:"noodles_small.jpg"},
{id:"noodle-l-spicy",name:"大涼粉（酸辣）",desc:"爽口彈滑",price:100,img:"noodles_large.jpg"},
{id:"noodle-l-sesame",name:"大涼粉（胡麻）",desc:"爽口彈滑",price:100,img:"noodles_large.jpg"},
...
const state=JSON.parse(localStorage.getItem("justEnoughCartV7")||"{}");
const list=document.querySelector("#product-list");
const tpl=document.querySelector("#product-template");

function save(){
  localStorage.setItem("justEnoughCartV7",JSON.stringify(state));
  renderBar();
}
...
function buildOrder(){
  ...
  selected().forEach(p=>{
    lines.push(`${p.name} × ${state[p.id].qty}　$${p.price*state[p.id].qty}`);
  });
...
function openSheet(){
  ...
  selected().forEach(p=>{
    ...
    d.innerHTML=`<div><strong>${p.name}</strong><small>數量 × ${state[p.id].qty}</small></div><strong>$${p.price*state[p.id].qty}</strong>`;
    wrap.appendChild(d);
  });
